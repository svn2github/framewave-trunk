                    ...
                    if(id>0)
                        {
                            ...
                            temp.d = _mm_sub_pd(temp.d, _mm_load_pd(&atanlo_ds[6]));// (x*(s1+s2) - atanlo[id]) // load and subtract                                    
                            ...
                            temp.i = _mm_or_si128(temp.i, _mm_slli_epi64(_mm_srli_epi64(src.i, 63), 63));// get the sign bits and change sign of result                                         
                    ...
                    if(id>0)
                        {
                            ...
                            temp.d = Atan_Summation_d(x.d);                          // returns x*(s1+s2)
                            //z    = atanhi[id] - ((x*(s1+s2) - atanlo[id]) - x);
                            temp.d = _mm_sub_pd(temp.d, _mm_load_pd(&atanlo_ds[4])); // (x*(s1+s2) - atanlo[id]) // load and subtract              
                            temp.d = _mm_sub_pd(temp.d, x.d);                        // (x*(s1+s2) - atanlo[id]) - x
            ...
            if(id>0)
                {
                    ...
                    temp.d = _mm_add_pd(src.d, src.d);                               // x + x
                    dst    = _mm_or_si128(dst,_mm_and_si128(temp.i, mask5));
                              //retain prev. dst // retain only masked    
                }
...
IS __m128 Atan_Summation_f(__m128 &x)
    {
        ...
        z     = _mm_mul_ps(x, x);           // z   = x*x;        
        w     = _mm_mul_ps(z,z);            // w   = z*z;
...
if(id>0)
{
    ...
    if(id>0)
        {
        mask5  = _mm_and_si128(mask4,mask1);                              // form the correct mask
        temp.f = _mm_add_ps(src.f, src.f);                                // x + x
        dst    = _mm_or_si128(dst,_mm_and_si128(temp.i, mask5));          // retain only masked and also retain prev. dst
        }
