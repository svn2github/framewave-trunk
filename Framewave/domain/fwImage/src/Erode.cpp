...
/*
Current idea to do the erode with arbitrary shape of SE
...
}	fwiErode_8u_In_Place;

#ifdef Hai_DEBUG
void printout_erode(const Fw8u* pSrc, int width, int height, int channel)
{
...
inline __m128i _My_mm_slli_si128_erode(__m128i xmm01, int imd)
{
	/*if(imd != 0)
	{
		for(int i = 0; i < imd; i++)
...
__m128i _My_mm_srli_si128_erode_erode(__m128i xmm01, __m128i* xmm031, int imd)
{
	xmm031 = xmm031;
...
__m128i _My_mm_srli_si128_erode_erode(__m128i xmm01, int imd)
{
	if(imd != 0)
	{
		for(int i = 0; i < imd; i++)
...
void erode_ref(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
	...
	for(int y = 0; y <= yEnd - yBegin; y++)
	{
		for(int x = 0; x < roiSize.width; x++)
		{
			for (int k=0; k<channel; k++)
			{
				...
				{
					*(pDst+y*dstStep+x*channel+k) = 255;
...
//void erode_ref(Fw8u** pSrc, Fw8u* pDst, int dstStep, const Fw8u* pMask_reflect, 
...
void erode_ref(Fw8u** pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					  const Fw8u* pMask_reflect, FwiSize maskSize, FwiPoint anchor, int channel, 
					   int xBegin, int xEnd, int yBegin, int yEnd)
{
	roiSize = roiSize;
	...
	for(int y = 0; y <= yEnd - yBegin; y++)
	{
		for(int x = xBegin; x < xEnd + anchor.x; x++)
		{
			for (int k=0; k<channel; k++)
			{
				...
				{
					*(pDst+y*dstStep+x*channel+k - anchor.x) = 255;
...
//Dilation ref in the bufferImage
void erode_ref_C1IR(Fw8u** pSrc, Fw8u* pDst, int dstStep, const Fw8u* pMask_reflect, 
...
void initResultMask_C1R_erode(Fw8u* resultMask, int padMaskWidth, FwiSize maskSize)
{
...
void initDataMask_C1IR_erode(Fw8u* resultMask, int padMaskWidth, int jumpStep)
{
	for(int i = 0; i < padMaskWidth; i++)
	{
		if(i < jumpStep)
...
void initResultMask_C3R_erode(Fw8u* resultMask, int padMaskWidth, FwiSize maskSize, int channel)
{
...
void initResultMask_C4R_erode(Fw8u* resultMask, int padMaskWidth, FwiSize maskSize, int channel)
{
	for(int i = 0; i < padMaskWidth; i+=channel)
	{
		if((i % (maskSize.width*channel) == 0) && (i < (padMaskWidth - maskSize.width*channel)))
...
//void copyOneLine_erode(const Fw8u* pSrc, Fw8u* pDst, int width, int channel)
//{
...
void copyOneLine_Erode(Fw8u* pSrc, Fw8u* pDst, int width, int channel)
{
	const Fw8u* ps  = pSrc; Fw8u* pd  = pDst;
	int xBegin = (16 - (int)((Fw64u)(pd))%16)%16;
	int xEnd = width * channel - (int)((Fw64u)(pd + width * channel) % 16);
...
void copyOneLine_Erode(Fw8u* pSrc, Fw8u* pDst, int width, int channel, int jumpStep)
{
	const Fw8u* ps  = pSrc; Fw8u* pd  = pDst;
	int xBegin = (16 - (int)((Fw64u)(pd))%16)%16;
	int xEnd = width * channel - (int)((Fw64u)(pd + width * channel) % 16);
...
void loadOneLineData_C1IR_erode(Fw8u* pSrc, int srcStep, Fw8u* pDst, int jumpStep, int xEnd, __m128i dmask)
{
	Fw8u* ps  = pSrc; 
	Fw8u* pd  = pDst;
	int x = 0;
...
void saveOneLineData_C1IR_erode(Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, int xEnd, int jumpStep)
{
	Fw8u* ps  = pSrc; Fw8u* pd  = pDst;
	int x = 0;
...
void shiftUpOneLine_erode(Fw8u** bufferImage, /*int width, */int height/*, int channel*/)
{
...
//void bufferErode_C1IR_16(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
...
//	erode_ref(pSrc, srcStep, pDst, dstStep, roiSize, pMask_reflect,
...
//	//initResultMask_C1R_erode(resultMask, pMask_pad_width, maskSize);
...
//					xmm00 = _mm_min_epu8(xmm00, xmm01);				//find the min of rows
//				}
//
//				//find the min of the row min
//				xmm01 = xmm00;
//
...
void setBufferLine255_erode(Fw8u* pDst, int dstStep, int channel)
{
	Fw8u* pd = pDst;
	int xEnd = dstStep - (int)((Fw64u)(pDst + dstStep) % 16);
	int x = 0/*, k = 0*/;
	__m128i xmm00 = _mm_setzero_si128();
	//Fw8u mask[16] = {255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255, 255};
	Fw8u *mask = (Fw8u*)fwMalloc(sizeof(__m128i));
	for(x = 0; x < 16; x++)
	{
		*(mask+x) = 255;
	}
...
void setBufferLine255_erode_unaligned(Fw8u* pDst, int dstStep, int channel)
{
	Fw8u* pd = pDst;
	int xEnd = dstStep - (int)((Fw64u)(pDst + dstStep) % 16);
	int x = 0/*, k = 0*/;
...
void setBufferLineZero_erode(Fw8u* pDst, int dstStep, int channel)
{
	Fw8u* pd = pDst;
	int xEnd = dstStep - (int)((Fw64u)(pDst + dstStep) % 16);
	int x = 0/*, k = 0*/;
...
void setBufferLineZero_erode_unaligned(Fw8u* pDst, int dstStep, int channel)
{
	Fw8u* pd = pDst;
	int xEnd = dstStep - (int)((Fw64u)(pDst + dstStep) % 16);
	int x = 0/*, k = 0*/;
...
__m128i loadDatabyShift_C1_erode(__m128i x, __m128i y)
{
...
bufferErode_C1IR_16(bufferImage, bufferResult[bufferResultHeight - 1], bufferResultWidth, maskSize, rmask, channel, xmm02, xmm03, xEnd);
*/
//void bufferErode_C1IR_16(Fw8u** pSrc, Fw8u* pDst, int dstStep, FwiSize maskSize,
...
//
//	//Initialize the pDst
//	setBufferLine255_erode(pDst, dstStep, channel);
//	xmm00 = _mm_setzero_si128();
//
//	ps = pSrc[0];  pd = pDst;
//
...
void bufferErode_C1IR_16(Fw8u** pSrc, Fw8u* pDst, int dstStep, FwiSize roiSize,
...
void bufferErode_C3IR_16(Fw8u** pSrc, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 Fw8u* pMask_pad, FwiSize maskSize, int pMask_pad_width,
					 __m128i rmask, int channel/*, int yBegin, int yEnd*/)
{
	channel = 3;
...
void bufferErode_AC4IR_16(Fw8u** pSrc, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 Fw8u* pMask_pad, FwiSize maskSize, int pMask_pad_width,
					 __m128i rmask, int channel/*, int yBegin, int yEnd*/)
{
	channel = 4;
...
//Erode (C1R), maskSize.width <= 16.
void erode_C1R_SSE2_16(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	*/
	...
	//Fw8u* shiftMask = (Fw8u*)fwMalloc(sizeof(Fw8u) * (pMask_pad_width * 1));
	//for(x = 0; x < 16; x++)
	//{
	//	shiftMask[x] = 255;
	//}
	//__m128i sMask = *(__m128i*)(shiftMask);
	...
	for(y = 0; y <= yEnd - yBegin; y++, pSrc += srcStep, pDst += dstStep)
	{
		...
		for(m = 0; m <= mEnd; m+=jumpStep, ps += jumpStep, pd += jumpStep)
		{
			...
			for(x = m; x < m + maskSize.width; x++, ps3++, pd3++)
			{
				...
				xmm01 = _mm_loadu_si128((__m128i*)ps3);				//load data
				xmm02 = *(__m128i*)(pMask);							//load mask
				xmm00 = _mm_andnot_si128(xmm01, xmm02);				//masking
				...
				for(z = 1; z < maskSize.height; z++, ps3 += srcStep, pMask += pMask_pad_width)	//16 might change
				{
					xmm01 = _mm_loadu_si128((__m128i*)ps3);			//load data
					xmm02 = *(__m128i*)(pMask);						//load mask
					xmm01 = _mm_andnot_si128(xmm01, xmm02);			//masking
					xmm00 = _mm_max_epu8(xmm00, xmm01);				//find the min of rows
				}
				...
				//xmm00 = _mm_and_si128(xmm00, rmask);
				xmm00 = _mm_andnot_si128(xmm00, rmask);
				xmm01 = _mm_loadu_si128((__m128i*)pd3);
				xmm00 = _mm_or_si128(xmm01, xmm00);
				_mm_storeu_si128((__m128i *)(pd3), xmm00);
...
//Erode (C1R), maskSize.width <= 32 && maskSize.width > 16.
void erode_C1R_SSE2_32(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	for(y = 0; y <= yEnd - yBegin; y++, pSrc += srcStep, pDst += dstStep)
	{
		...
		for(m = 0; m <= mEnd; m+=jumpStep, ps += jumpStep, pd += jumpStep)
		{
			...
			for(x = m; x < m + maskSize.width; x++, ps3++, pd3++)
			{
				...
				xmm010 = _mm_andnot_si128(xmm010, xmm020);			//masking
				xmm011 = _mm_andnot_si128(xmm011, xmm021);			//masking
				xmm00 = _mm_or_si128(xmm010, xmm011);				//masking
				...
				for(z = 1; z < maskSize.height; z++, ps3 += srcStep, pMask += pMask_pad_width)	//16 might change
				{
					...
					xmm00 = _mm_max_epu8(xmm00, xmm010);				//find the min of rows
				}
...
*/
//void erode_C1IR_SSE2_16(Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
//					 const Fw8u* pMask_reflect, Fw8u* pMask_pad, FwiSize maskSize, int pMask_pad_width,
//					 FwiPoint anchor, int channel, int yBegin, int yEnd)
//{
//	channel = 1;
...
//	initDataMask_C1IR_erode(dataMask, pMask_pad_width, jumpStep);
...
//		loadOneLineData_C1IR_erode(pSrc, (roiSize.width + maskSize.width - 1),
...
//
//		//Move one line up
//		shiftUpOneLine_erode(bufferImage, maskSize.height);
//	}
//
...
//
//		//dilation SSE2
//		bufferErode_C1IR_16(bufferImage, bufferResult[bufferResultHeight - 1], 
...
//		//dilation ref
//		//erode_ref_C1IR(bufferImage, bufferResult[bufferResultHeight-1], roiSize.width, pMask_reflect, maskSize, anchor, xEnd + jumpStep, roiSize.width, y, y);
//
//		//printout(pSrc, roiSize.width + maskSize.width - 1, 1, 1);
//		//printout(bufferImage[maskSize.height - 1], bufferImageWidth, 1, 1);
//
//		shiftUpOneLine_erode(bufferResult, bufferResultHeight);
//		shiftUpOneLine_erode(bufferImage, maskSize.height);
//	}
//
//	loadOneLineData_C1IR_erode(pSrc, (roiSize.width + maskSize.width - 1),
...
//
//		saveOneLineData_C1IR_erode(bufferResult[0], bufferImageWidth, pSrc - backOffset, bufferResultWidth, xEnd, jumpStep);
...
//		loadOneLineData_C1IR_erode(pSrc + srcStep, (roiSize.width + maskSize.width - 1),
//			bufferImage[maskSize.height - 1], jumpStep, xEnd, dmask);
//
//		//dilation SSE2
//		bufferErode_C1IR_16(bufferImage, bufferResult[bufferResultHeight - 1],
					 ...
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	{
		//copy the data to the bottom of the bufferImage.
		copyOneLine_Erode(pSrc, bufferImage[maskSize.height - 1], bufferImageWidth, channel);
	...
	for(y = yBegin + maskSize.height; y < yBegin + maskSize.height + bufferResultHeight - 1; y++, pSrc += srcStep)
	{
		copyOneLine_Erode(pSrc, bufferImage[maskSize.height - 1], bufferImageWidth, channel);
		//dilation SSE2
		bufferErode_C1IR_16(bufferImage, bufferResult[bufferResultHeight - 1], roiSize.width * channel, roiSize, pMask_pad, maskSize, pMask_pad_width, rmask, channel);
		//dilation ref
		erode_ref(bufferImage, srcStep, bufferResult[bufferResultHeight-1], roiSize.width, roiSize, pMask_reflect, maskSize, anchor, channel, mEnd + jumpStep, roiSize.width, y, y);
	...
	{
		//copy the result to the pSrc where the data will not be used any more.
		copyOneLine_Erode(bufferResult[0], pSrc - backOffset, roiSize.width, channel);
		shiftUpOneLine_erode(bufferResult, bufferResultHeight);
		shiftUpOneLine_erode(bufferImage, maskSize.height);
		//copy the data from the pSrc to the bufferImage.
		copyOneLine_Erode(pSrc + srcStep, bufferImage[maskSize.height - 1], bufferImageWidth, channel);
		//dilation SSE2
		bufferErode_C1IR_16(bufferImage, bufferResult[bufferResultHeight - 1], roiSize.width * channel, roiSize, pMask_pad, maskSize, pMask_pad_width, rmask, channel);
		//dilation ref
		erode_ref(bufferImage, srcStep, bufferResult[bufferResultHeight-1], roiSize.width, roiSize, pMask_reflect, maskSize, anchor, channel, mEnd + jumpStep, roiSize.width, y, y);
...
void erode_C3IR_SSE2_16(Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	Fw8u* resultMask = (Fw8u*) fwMalloc(sizeof(Fw8u) * (pMask_pad_width * 1));
	initResultMask_C3R_erode(resultMask, pMask_pad_width, maskSize, channel);
	__m128i rmask = *(__m128i*)(resultMask);
	...
	for(y = yBegin + maskSize.height; y < yBegin + maskSize.height + bufferResultHeight - 1; y++, pSrc += srcStep)
	{
		copyOneLine_Erode(pSrc, bufferImage[maskSize.height - 1], bufferImageWidth, channel);
		//dilation SSE2
		bufferErode_C3IR_16(bufferImage, bufferResult[bufferResultHeight - 1], roiSize.width * channel, roiSize, pMask_pad, maskSize, pMask_pad_width, rmask, channel);
		//dilation ref
		erode_ref(bufferImage, srcStep, bufferResult[bufferResultHeight-1], roiSize.width, roiSize, pMask_reflect, maskSize, anchor, channel, mEnd + jumpStep, roiSize.width, y, y);
...
void erode_AC4IR_SSE2_16(Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad, Fw8u* pMask_negative, 
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	channel = 4;
	...
	Fw8u* resultMask = (Fw8u*) fwMalloc(sizeof(Fw8u) * (pMask_pad_width * 1));
	initResultMask_C4R_erode(resultMask, pMask_pad_width, maskSize, channel);
	__m128i rmask = *(__m128i*)(resultMask);
	...
	for(y = yBegin + maskSize.height; y < yBegin + maskSize.height + bufferResultHeight - 1; y++, pSrc += srcStep)
	{
		copyOneLine_Erode(pSrc, bufferImage[maskSize.height - 1], bufferImageWidth, channel);
		//dilation SSE2
		bufferErode_AC4IR_16(bufferImage, bufferResult[bufferResultHeight - 1], roiSize.width * channel, roiSize, pMask_pad, maskSize, pMask_pad_width, rmask, channel);
		//dilation ref
		erode_ref(bufferImage, srcStep, bufferResult[bufferResultHeight-1], roiSize.width, roiSize, pMask_reflect, maskSize, anchor, channel, mEnd + jumpStep, roiSize.width, y, y);
...
//Erode (C3R), maskSize.width <= 5.
void erode_C3R_SSE2_16(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad, 
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
...
//Erode (C4R), maskSize.width <= 4.
void erode_C4R_SSE2_16(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad, FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	channel = 4;
...
//Erode (AC4R), maskSize.width <= 4.
void erode_AC4R_SSE2_16(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
...
//Erode (C3R), 6 <= maskSize.width <= 10
void erode_C3R_SSE2_32(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	for(y = 0; y <= yEnd - yBegin; y++, pSrc += srcStep, pDst += dstStep)
	{
		...
		for(m = 0; m <= mEnd; m+=jumpStep, ps += jumpStep, pd += jumpStep)
		{
			...
			for(x = m; x < m + maskSize.width; x++, ps3+= channel, pd3+= channel)
			{
				...
				xmm00 = _mm_max_epu8(xmm020, xmm010);				//find the min of rows
...
//Erode (C4R), 4 < maskSize.width <= 8
void erode_C4R_SSE2_32(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad,
					 FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	...
	for(y = 0; y <= yEnd - yBegin; y++, pSrc += srcStep, pDst += dstStep)
	{
		...
		for(m = 0; m <= mEnd; m+=jumpStep, ps += jumpStep, pd += jumpStep)
		{
			...
			for(x = m; x < m + maskSize.width; x++, ps3+= channel, pd3+= channel)
			{
				...
				//xmm021 = _mm_slli_si128(xmm011, 1);					//shifting
				//xmm020 = _mm_or_si128(xmm020, xmm021);				//Obtain the red byte back
				xmm00 = _mm_max_epu8(xmm010, xmm011);				//find the min of rows
...
//Erode (AC4R), 4 < maskSize.width <= 8
void erode_AC4R_SSE2_32(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize,
					 const Fw8u* pMask_reflect, Fw8u* pMask_pad, FwiSize maskSize, int pMask_pad_width,
					 FwiPoint anchor, int channel, int yBegin, int yEnd)
{
	channel = 4;
...
void erode_ref(const Fw32f* pSrc, int srcStep, Fw32f* pDst, int dstStep, FwiSize roiSize,
					   Fw8u* pMask_reflect, FwiSize maskSize, FwiPoint anchor, int channel)
{
//Erode with specified square mask.
...
void padMask_16_C1R_erode(const Fw8u* pMask, Fw8u* pMask_pad, int pMask_padWidth, FwiSize maskSize)
{
	int counter = 0;
	int x = 0, y = 0;
...
//Different than the padMask_16_C1R_erode, this function will not circulate the mask.
void padMask_32_C1R_erode(const Fw8u* pMask, Fw8u* pMask_pad, int pMask_padWidth, FwiSize maskSize)
{
...
void padMask_16_C3R_erode(const Fw8u* pMask, Fw8u* pMask_pad, int pMask_padWidth, FwiSize maskSize)
{
	for(int y = 0; y < maskSize.height; y++)
	{
		for(int x = 0; x < pMask_padWidth; x++)
...
void padMask_16_C4R_erode(const Fw8u* pMask, Fw8u* pMask_pad, int pMask_padWidth, FwiSize maskSize)
{
	for(int y = 0; y < maskSize.height; y++)
	{
		for(int x = 0; x < pMask_padWidth; x++)
...
void My_fwiErode_8u_C1R(void* param)
{
	fwiErode_8u_Not_In_Place *erode_par = (fwiErode_8u_Not_In_Place *) param;

	const Fw8u* pSrc = erode_par->pSrc + erode_par->yBegin * erode_par->srcStep;
	int srcStep = erode_par->srcStep;
	Fw8u* pDst = erode_par->pDst + erode_par->yBegin * erode_par->dstStep;
	int dstStep = erode_par->dstStep;
	FwiSize roiSize = erode_par->roiSize;
	int channel = erode_par->channel;
	int yBegin = erode_par->yBegin;
	int yEnd = erode_par->yEnd;
	const Fw8u* pMask = erode_par->pMask;
	FwiSize maskSize = erode_par->maskSize;
	FwiPoint anchor = erode_par->anchor;
	...
	//Fw8u* pMask_negative = (Fw8u*) fwMalloc(sizeof(Fw8u) * pMask_pad_width * channel * maskSize.height);

	if(maskSize.width <= 16)
	{
		padMask_16_C1R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		erode_C1R_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 16 && maskSize.width <= 32)
	{
		padMask_32_C1R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		erode_C1R_SSE2_32(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 32 && maskSize.width <= 64)
		pMask_pad_width = 64;
	else if (maskSize.width > 64 && maskSize.width <= 128)
	...
	fwFree(pMask_pad);
	//fwFree(pMask_negative);
}

void My_fwiErode_8u_C1IR(void* param)
{
	fwiErode_8u_In_Place *erode_par = (fwiErode_8u_In_Place *) param;

	Fw8u* pSrc = erode_par->pSrc + erode_par->yBegin * erode_par->srcStep;
	int srcStep = erode_par->srcStep;
	Fw8u* pDst = erode_par->pDst + erode_par->yBegin * erode_par->dstStep;
	int dstStep = erode_par->dstStep;
	FwiSize roiSize = erode_par->roiSize;
	...
	FwiSize maskSize = erode_par->maskSize;
	FwiPoint anchor = erode_par->anchor;
	//Fw8u* remainBottomLines = erode_par->remainBottomLines;
	//Fw8u* remainTopLines = erode_par->remainTopLines;
	...
	{
		padMask_16_C1R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		erode_C1IR_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 16 && maskSize.width <= 32)
	{
		padMask_32_C1R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
...
void My_fwiErode_8u_C3R(void* param)
{
	fwiErode_8u_Not_In_Place *erode_par = (fwiErode_8u_Not_In_Place *) param;
	...
	if(maskSize.width <= 5)
	{
		...
		padMask_16_C3R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		erode_C3R_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 5 && maskSize.width <= 10)
	{
		padMask_16_C3R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		erode_C3R_SSE2_32(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 10 && maskSize.width <= 16)
	{
		pMask_pad_width = 48;
...
void My_fwiErode_8u_C4R(void* param)
{
	fwiErode_8u_Not_In_Place *erode_par = (fwiErode_8u_Not_In_Place *) param;
	...
	//pad the mask with 0's
	Fw8u* pMask_pad = (Fw8u*) fwMalloc(sizeof(Fw8u) * pMask_pad_width * maskSize.height);
	//Fw8u* pMask_negative = (Fw8u*) fwMalloc(sizeof(Fw8u) * pMask_pad_width * maskSize.height);

	if(maskSize.width <= 4)
	{
		//Circulate padding the mask to fit into 16 bytes SSE2 register
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_C4R_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 4 && maskSize.width <= 8)
	{
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_C4R_SSE2_32(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 8 && maskSize.width <= 12)
	{
		pMask_pad_width = 48;
...
void My_fwiErode_8u_AC4R(void* param)
{
	fwiErode_8u_Not_In_Place *erode_par = (fwiErode_8u_Not_In_Place *) param;
	...
	if(maskSize.width <= 4)
	{
		...
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_AC4R_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 4 && maskSize.width <= 8)
	{
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_AC4R_SSE2_32(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 8 && maskSize.width <= 12)
	{
		pMask_pad_width = 48;
...
void My_fwiErode_8u_C3IR(void* param)
{
	fwiErode_8u_In_Place *erode_par = (fwiErode_8u_In_Place *) param;
	...
	if(maskSize.width <= 5)
	{
		...
		padMask_16_C3R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_C3IR_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 5 && maskSize.width <= 10)
	{
		padMask_16_C3R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
...
void My_fwiErode_8u_AC4IR(void* param)
{
	fwiErode_8u_In_Place *erode_par = (fwiErode_8u_In_Place *) param;
	...
	if(maskSize.width <= 4)
	{
		...
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
		//printout(pMask_pad, pMask_pad_width, maskSize.height, 1);
		erode_AC4IR_SSE2_16(pSrc, srcStep, pDst, dstStep, roiSize, pMask, pMask_pad, pMask_negative, maskSize, pMask_pad_width, anchor, channel, yBegin, yEnd);
	}
	else if (maskSize.width > 4 && maskSize.width <= 8)
	{
		padMask_16_C4R_erode(pMask, pMask_pad, pMask_pad_width, maskSize);
...
FwStatus MyFW_Erode(const TS* pSrc, int srcStep, TS* pDst, int dstStep, FwiSize roiSize,
...
template< class TS, CH chSrc, DispatchType disp >	
FwStatus MyFW_Erode(TS* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	...
	//if((maskSize.width-1+roiSize.width)*channel > srcDstStep) return fwStsStrideErr;
...
FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_C1R)(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		...
		case DT_SSE2:
		{
			...
			*/
			...
			fwiErode_8u_Not_In_Place erode_par[MAXTHREADNUM];
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					erode_par[thread_num].pSrc = pSrc;
					erode_par[thread_num].srcStep = srcStep;
					erode_par[thread_num].pDst = pDst;
					erode_par[thread_num].dstStep = dstStep;
					erode_par[thread_num].roiSize = roiSize;
					erode_par[thread_num].yBegin = y_bgn;
					erode_par[thread_num].yEnd = y_end;
					erode_par[thread_num].channel = channel;
					//erode_par[thread_num].pMask = pMask_reflect;
					erode_par[thread_num].pMask = pMask;
					erode_par[thread_num].maskSize = maskSize;
					erode_par[thread_num].anchor = anchor;

					ThreadPool::Run(My_fwiErode_8u_C1R, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, C1, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_C3R)(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
			...
			*/
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					ThreadPool::Run(My_fwiErode_8u_C3R, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, C3, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_C4R)(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
			...
			*/
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					ThreadPool::Run(My_fwiErode_8u_C4R, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, C4, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_AC4R)(const Fw8u* pSrc, int srcStep, Fw8u* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
			...
			*/
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					ThreadPool::Run(My_fwiErode_8u_AC4R, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, AC4, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_C1R)(const Fw32f* pSrc, int srcStep, Fw32f* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	return MyFW_Erode<Fw32f, C1, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_C3R)(const Fw32f* pSrc, int srcStep, Fw32f* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	return MyFW_Erode<Fw32f, C3, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_C4R)(const Fw32f* pSrc, int srcStep, Fw32f* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	return MyFW_Erode<Fw32f, C4, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_AC4R)(const Fw32f* pSrc, int srcStep, Fw32f* pDst, int dstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor) 
{
	return MyFW_Erode<Fw32f, AC4, DT_REFR> (pSrc, srcStep, pDst, dstStep, roiSize, pMask, maskSize, anchor);
}

//The idea will be use C1R to get the Dilation results, then copy to a temporary buffer. Finally, copy back to pSrc.
FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_C1IR)(Fw8u* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
		case DT_SSE2:
		{
			...
			*/
			...
			fwiErode_8u_In_Place erode_par[MAXTHREADNUM];
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					erode_par[thread_num].pSrc = pSrcDst;
					erode_par[thread_num].srcStep = srcDstStep;
					//erode_par[thread_num].pDst = t_pDst;
					//erode_par[thread_num].dstStep = roiSize.width * channel;
					erode_par[thread_num].roiSize = roiSize;
					erode_par[thread_num].yBegin = y_bgn;
					erode_par[thread_num].yEnd = y_end;
					erode_par[thread_num].channel = channel;
					...
					erode_par[thread_num].maskSize = maskSize;
					erode_par[thread_num].anchor = anchor;
					erode_par[thread_num].remainBottomLines = remainBottomLines[thread_num];
					erode_par[thread_num].remainTopLines = remainTopLines[thread_num];

					ThreadPool::Run(My_fwiErode_8u_C1IR, (void *) &erode_par[thread_num]);
				}
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					copyOneLine_Erode(remainTopLines[thread_num], pSrcDst + (erode_par[thread_num].yBegin) * srcDstStep, 
						roiSize.width, channel);
					copyOneLine_Erode(remainBottomLines[thread_num], pSrcDst + (erode_par[thread_num].yEnd) * srcDstStep, 
						roiSize.width, channel);
				}
			}
		...
		default:
			return MyFW_Erode<Fw8u, C1, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_C3IR)(Fw8u* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
			...
			*/
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					ThreadPool::Run(My_fwiErode_8u_C3IR, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, C3, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_8u_AC4IR)(Fw8u* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	switch( Dispatch::Type<DT_SSE2>())
	{
		case DT_SSE3:
			...
			*/
			...
			if(total_thread_num >= 1)
			{
				...
				for(thread_num=0; thread_num<total_thread_num; thread_num++)
				{
					...
					ThreadPool::Run(My_fwiErode_8u_AC4IR, (void *) &erode_par[thread_num]);
				}
		...
		default:
			return MyFW_Erode<Fw8u, AC4, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
	}
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_C1IR)(Fw32f* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	return MyFW_Erode<Fw32f, C1, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_C3IR)(Fw32f* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	return MyFW_Erode<Fw32f, C3, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
}

FwStatus PREFIX_OPT(OPT_PREFIX, fwiErode_32f_AC4IR)(Fw32f* pSrcDst, int srcDstStep, FwiSize roiSize, const Fw8u* pMask, FwiSize maskSize, FwiPoint anchor)
{
	return MyFW_Erode<Fw32f, AC4, DT_REFR> (pSrcDst, srcDstStep, roiSize, pMask, maskSize, anchor);
}
